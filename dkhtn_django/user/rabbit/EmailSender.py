...
def send_email(email_ses):
    message = json.loads(email_ses)
    email = message['email']
    session_id = message['session_id']
    sms_code = '%06d' % random.randint(0, 999999)

    redis_utils.redis_set(settings.REDIS_DB_VERIFY, session_id, sms_code, 60 * 15)
...
class AMQPConsuming(threading.Thread):
    def callback(self, ch, method, properties, email_ses):
        # do something
        send_email(email_ses.decode("utf-8"))
